...
def change_thresholds(step, system_runner: interfaces.IRunner, communicator: interfaces.ICommunicator, leader):
    ...
    if step == (system_runner._integrator.ntcmd / system_runner._options.timesteps):
        initial_row = system_runner._integrator.ntcmdprep / system_runner._options.timesteps
    elif step == ((system_runner._integrator.ntcmd + system_runner._integrator.nteb) / system_runner._options.timesteps): 
        initial_row = (system_runner._integrator.ntcmd + system_runner._integrator.ntebprep) / system_runner._options.timesteps

    if initial_row != 0:
        gamd_log_filename = os.path.join("Logs", f"gamd_{system_runner._rank:03d}.log")

        if system_runner._integrator._GamdStageIntegrator__boost_type.name == "TOTAL" or system_runner._integrator._GamdStageIntegrator__boost_type.name == "DUAL_TOTAL_DIHEDRAL":
            ...
            tot_threshold_sd = [system_runner._simulation.integrator.getGlobalVariableByName("threshold_energy_Total"), tot_sd]
            if leader == True:
                # gather energy thresholds and widths
                tot_thresholds = communicator.gather_thresholds_from_workers(tot_threshold_sd)
                # set new thresholds
                tot_new_threshold = new_thresholds(tot_thresholds)
                tot_threshold = communicator.distribute_thresholds_to_workers(tot_new_threshold)
            else:
        ...
        if system_runner._integrator._GamdStageIntegrator__boost_type.name == "DIHEDRAL" or system_runner._integrator._GamdStageIntegrator__boost_type.name == "DUAL_TOTAL_DIHEDRAL": 
            column = 3 # Unboosted-Dihedral-Energy

            # calculate standard deviation of the energy (width)
            dih_sd = compute_energy_width(gamd_log_filename, initial_row, column)
                 
            dih_threshold_sd = [system_runner._simulation.integrator.getGlobalVariableByName("threshold_energy_Dihedral"), dih_sd]
            if leader == True:
                # gather energy thresholds and widths
                dih_thresholds = communicator.gather_thresholds_from_workers(dih_threshold_sd)
                # set new threshold
                dih_new_threshold = new_thresholds(dih_thresholds)
                dih_threshold = communicator.distribute_thresholds_to_workers(dih_new_threshold)
            else:
...
def compute_energy_width(gamd_log_filename, initial_row, column):
